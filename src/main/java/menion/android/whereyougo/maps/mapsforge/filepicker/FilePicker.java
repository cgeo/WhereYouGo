...
/**
 * A FilePicker displays the contents of directories. The user can navigate within the file system
 * and select a single file whose path is then returned to the calling activity. The ordering of
 * directory contents can be specified via {@link #setFileComparator(Comparator)}. By default
 * subfolders and files are grouped and each group is ordered alphabetically.
 * <p>
 * A {@link FileFilter} can be activated via {@link #setFileDisplayFilter(FileFilter)} to restrict
 * the displayed files and folders. By default all files and folders are visible.
 * <p>
 * Another <code>FileFilter</code> can be applied via {@link #setFileSelectFilter(ValidFileFilter)}
 * to check if a selected file is valid before its path is returned. By default all files are
 * considered as valid and can be selected.
 */
public class FilePicker extends Activity implements AdapterView.OnItemClickListener {
  ...
  /**
   * Sets the file display filter. This filter is used to determine which files and subfolders of
   * directories will be displayed. If set to null, all files and subfolders are shown.
   * 
  ...
  /**
   * Sets the file select filter. This filter is used when the user selects a file to determine if
   * it is valid. If set to null, all files are considered as valid.
   * 
