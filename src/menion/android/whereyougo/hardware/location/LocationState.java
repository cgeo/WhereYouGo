...
public class LocationState {
  ...
  public static Location getLastKnownLocation(Activity activity) {
    ...
    try {
      gpsLocation = LocusUtils.convertToL(lm.getLastKnownLocation(LocationManager.GPS_PROVIDER));
    ...
    try {
      networkLocation =
          LocusUtils.convertToL(lm.getLastKnownLocation(LocationManager.NETWORK_PROVIDER));
    } catch (SecurityException e) {
      Logger.w(TAG, "Failed to retrieve location: access appears to be disabled.");
    } catch (IllegalArgumentException e) {
    ...
    if (!mListeners.contains(listener)) {
      ...
      if (mListeners.size() > 0)
        Collections.sort(mListeners, new Comparator<LocationEventListener>() {
          public int compare(LocationEventListener object1, LocationEventListener object2) {
  ...
  public static void onActivityPauseInstant(Context context) {
    try {
      ...
      boolean disableWhenHide =
          context == null ? false : Settings.getPrefBoolean(context,
